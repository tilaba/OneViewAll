...
class PoseEvaluator:
    ...
    def _load_rcbv_meta(self, dataset_root):
      meta_path = os.path.join(dataset_root, 'models_cad/models_info.json')
...
def run_pose_estimation():
    ...
    video_dirs = sorted(glob.glob(f'{opt.tless_dir}/tless_test_primesense_bop19/test_primesense/*'))
    res = NestDict()
    ...
    evaluator = PoseEvaluator(opt.tless_dir, use_adds=opt.use_adds == 1)
    reader_tmp = TlessReader(video_dirs[0])
    ...
    for ob_id in ob_ids:
        # if ob_id != 2:  # TODO: remove this to test all objects
        #     break
        ...
        for video_dir in video_dirs:
            reader = TlessReader(video_dir, zfar=1.5)
    ...
    for ob_id, errors in error_dict.items():
        errors_np = np.array(errors)  # 归一化误差到相对直径
        thresholds = np.linspace(0, 0.2, 100)  # 0~0.1比例区间
        recalls = [np.mean(errors_np < t) for t in thresholds]
        ar_score = np.mean(recalls)
        print(f"Object {ob_id:02d} - Recall (error < 0.1 × diameter): {ar_score * 100:.2f}% "
        f"({(errors_np < 0.2).sum()}/{len(errors_np)})")

    mean_recall = np.mean(all_recalls)
    print(f"\nMean Recall 0.2 over all objects: {mean_recall * 100:.2f}%")


    for ob_id, errors in error_dict.items():
        errors_np = np.array(errors)  # 归一化误差到相对直径
        thresholds = np.linspace(0, 0.3, 100)  # 0~0.1比例区间
        recalls = [np.mean(errors_np < t) for t in thresholds]
        ar_score = np.mean(recalls)
        print(f"Object {ob_id:02d} - Recall (error < 0.1 × diameter): {ar_score * 100:.2f}% "
        f"({(errors_np < 0.3).sum()}/{len(errors_np)})")

    mean_recall = np.mean(all_recalls)
    print(f"\nMean Recall 0.3 over all objects: {mean_recall * 100:.2f}%")

    for ob_id, errors in error_dict.items():
      errors_np = np.array(errors)  # 归一化误差到相对直径
      thresholds = np.linspace(0, 0.4, 100)  # 0~0.1比例区间
      recalls = [np.mean(errors_np < t) for t in thresholds]
      ar_score = np.mean(recalls)
      print(f"Object {ob_id:02d} - Recall (error < 0.1 × diameter): {ar_score * 100:.2f}% "
      f"({(errors_np < 0.4).sum()}/{len(errors_np)})")
    ...
    for ob_id, errors in error_dict.items():
      errors_np = np.array(errors)  # 归一化误差到相对直径
      thresholds = np.linspace(0, 0.5, 100)  # 0~0.1比例区间
      recalls = [np.mean(errors_np < t) for t in thresholds]
      ar_score = np.mean(recalls)
      print(f"Object {ob_id:02d} - Recall (error < 0.1 × diameter): {ar_score * 100:.2f}% "
      f"({(errors_np < 0.5).sum()}/{len(errors_np)})")
...
if __name__ == '__main__':
    ...
    code_dir = os.path.dirname(os.path.realpath(__file__))
    parser.add_argument('--tless_dir', type=str, default="/mnt/YCB_Video", help="data dir")
    ...
    os.environ["YCB_VIDEO_DIR"] = opt.tless_dir
    set_seed(0)
