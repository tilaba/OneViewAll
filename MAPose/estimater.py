...
class FoundationPose:
  def __init__(self, model_pts, model_normals, symmetry_tfs=None, mesh=None, scorer:ScorePredictor=None, refiner:PoseRefinePredictor=None, glctx=None, debug=0, debug_dir='/home/bowen/debug/novel_pose_debug/'):
    ...
    self.make_rotation_grid(min_n_views=40, inplane_step=3)
  ...
  def register(self, K, rgb, depth, ob_mask, ob_id=None, glctx=None, iteration=5):
    ...
    # we use gpu to accelerate this function
    xyz_map = depth2xyzmap(depth, K)
    predict_start = time.time()
    poses, vis = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=poses.data.cpu().numpy(), normal_map=normal_map, xyz_map=xyz_map, ob_mask=ob_mask,glctx=self.glctx, mesh_diameter=self.diameter, iteration=iteration, get_vis=self.debug>=2, ob_id=ob_id)

    predict_time = time.time() - predict_start
    logging.info(f' refine took {predict_time:.4f}s')
    # if vis is not None:
    ...
    score_time = time.time() - scorer_start
    logging.info(f' score took {score_time:.4f}s')
    # if vis is not None:score_time
    #   imageio.imwrite(f'{self.debug_dir}/vis_score.png', vis)

    register_time = time.time() - register_start
    logging.info(f'register took {register_time:.4f}s')
  ...
  def track_one(self, rgb, depth,  ob_mask, K, iteration, extra={}):
    ...
    pose, vis, _ = self.refiner.predict(mesh=self.mesh, mesh_tensors=self.mesh_tensors, rgb=rgb, depth=depth, K=K, ob_in_cams=self.pose_last.reshape(1,4,4).data.cpu().numpy(), normal_map=None, xyz_map=xyz_map, ob_mask=ob_mask, mesh_diameter=self.diameter, glctx=self.glctx, iteration=iteration, get_vis=self.debug>=2)
